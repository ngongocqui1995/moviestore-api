...
function getAllEpisodesNotPaging(data, index){
    ...
    if(divClassInnerSectionPage !== undefined){
...
function getLinkVideo(data){
    let links = []
    const dom = new JSDOM(data)
    let tagScripts = dom.window.document.body.innerHTML
...
function getInformationCollection(data){
    ...
    // lấy hình ảnh bự trên cùng
    let imgMain = ""
    let divClassFluidImg = dom.window.document.getElementsByClassName('fluid-img')[0]
    if(divClassFluidImg !== undefined){
        imgMain = divClassFluidImg.getElementsByTagName('img')[0].getAttribute('src')
    }
