...
exports.getLinkVideo = async(req, res) => {
    let keyCollection = req.params.keyCollection
    let codeCollection = req.params.codeCollection
    let response = await axios.get(`https://tv.zing.vn/video/${keyCollection}/${codeCollection}.html`)
    let data = response.data
    if(data){   
...
function getLinkVideo(data){
    ...
    for(let i=0; i<links.length; i++){
        if(links[i].link.indexOf("http") !== -1){
            indexbd = links[i].link.indexOf("http")
            indexkt = links[i].link.indexOf("\"", indexbd)
            links[i].link = links[i].link.substring(indexbd, indexkt)
        }else{
            links.splice(i, 1)
        }
    }
...
function getInformationCollection(data){
    ...
    let tag = getItemsTag(divClassTag)

    // lấy thông tin thể loại
    let divClassCategories = divClassBoxDescription.getElementsByClassName('tag')[1]
    let categories = getItemsTagCategories(divClassCategories)

    let description = divClassBoxDescription.getElementsByClassName('rows2')[0].textContent.trim()

    // lấy thông tin see all
    let episodesAll = dom.window.document.getElementsByClassName('see-all')[0].getElementsByTagName('a')[0].getAttribute('href')
